...
### Stealth Mode - MutationObserver Interception

GRAPES includes a sophisticated stealth system that prevents websites from detecting extension modifications:

#### How It Works (Security Camera Loop Analogy)
Like a pre-recorded loop on a security camera showing the original scene instead of the live feed, the stealth system shows websites the "original" DOM state while the user sees the modified version.

#### Implementation (`entrypoints/stealth.content.ts`)

1. **Runs in MAIN World**: The script executes in the page's JavaScript context, before any page scripts, by using `world: 'MAIN'` in Manifest V3.

2. **MutationObserver Proxy**: Replaces the global `MutationObserver` with a filtered version that:
   - Intercepts all DOM change observations
   - Filters out mutations related to GRAPES elements (identified by `grapes-` prefix)
   - Only passes non-GRAPES mutations to the website's observers

3. **DOM Query Interception**: Overrides key DOM methods to hide GRAPES elements:
   - `querySelector` / `querySelectorAll`
   - `getElementById`
   - `childNodes` / `children` getters
   - `innerHTML` getter (for `<head>` element)

4. **Element Identification**: GRAPES elements are identified by:
   - ID containing `grapes-` prefix
   - Classes containing `grapes-` prefix  
   - `data-grapes-*` attributes

#### Security Implications
- Websites cannot detect that modifications have been made
- Anti-fingerprinting: Extension presence is not detectable via DOM inspection
- Page scripts see the "clean" original DOM structure
